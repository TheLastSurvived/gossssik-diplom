...
import useDebounceEffect from "../hook/useDebounceCallback";
...
const Template = () => {
    ...
    () =>
      [...stageData]?.sort((a, b) => {
        if (a.attrs.zIndex === b.attrs.zIndex) {
          if (a.attrs.zIndex < 0) {
            console.log("1", b.attrs.updatedAt - a.attrs.updatedAt);
            return b.attrs.updatedAt - a.attrs.updatedAt;
          }
          console.log("2", a.attrs.updatedAt - b.attrs.updatedAt);
          return a.attrs.updatedAt - b.attrs.updatedAt;
        }
        console.log("3", a.attrs.zIndex - b.attrs.zIndex);
        return a.attrs.zIndex - b.attrs.zIndex;
      }),
    [stageData],
  );

  const stageData1 = useSelector(stageDataSelector.selectAll);
  ...
  const save = async () => {
    ...
    await $api.put(`/template/${id}`, data).then((data) => {
  ...
  const header = (
    <Header>
      ...
      /> */}
      <h5>{title || "Неназванный шаблон"}</h5>
    </Header>
  );
  ...
  useEffect(() => {
    $api.get(`/template/${id}`).then((res) => {
      ...
      if (user !== null && res?.data?.author !== user?.user?.id) {
        return navigate("/templates");
      }
      if (res?.data?.data?.stageData) {
        dispatch(stageDataAction.addItem(res?.data?.data?.stageData));
      }
      if (res?.data?.data?.stageDataList[0]) {
        dispatch(stageDataListAction.updateItem(res?.data?.data?.stageDataList[0]));
      }
